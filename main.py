    ...
    for i in range (rows):
        ...
        # such as name, number, e mail id, dob, category etc
            if j == 0:
                ...
                if temp[j] == '' or temp[j] == ' ':
                    sys.exit("Name is a mandetory field. Process exitig due to blank field...")
...
def menu():
    # WE created this simple meu function for 
    # code reusability & also for an interactive console
    # Menu fuc will only execute when called
    print("*******************************")
    print("\t\t\tSMARTPHONE DIRECTORY", flush=False)
    print("*******************************")
    prit("\tYou can now perform the following operations on this phonebook\n")
    print("1. add a new contact")
    print("2. Remove an existing cotact")
    print("3. Delete all contacts")
    print("4. Search for a contact")
    print("5. Display all contacts")
    print("6. Exit phoebook")
...
def add_contact(pb):
    ...
    return pb
def remove_existig(pb):

    query = str(input("PLease eter the name of the contact you wish to remove: "))
    ...
    for i in range(len(pb)):
        if query == pb[i][0]:
            ...
            return pb
    if temp == 0:
        print("Sorry, youy have entered an ivalid query.\nPlease recheck and try agai later.")
        return pb

def delete_all(pb):
    return pb.clear()

def search_existig(pb):
...
print("***********************************")
print("Hello dear user, welcome to our samrtphone directory system")
print("You may now procced to explore this directory")
print("***************************************")
